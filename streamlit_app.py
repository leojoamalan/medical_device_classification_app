        ...
        if 0.5 < aspect_ratio < 2.0 and area > 1000:
            ...
            for (box, text, prob) in results:
                ...
                try:
                    value = float(numeric_text)
                    if 20 <= value <= 600:
                        device_type = 'glucometer'  # Assuming values are for glucometer
                        glucose_values.append(value)
                        break
                except ValueError:
                    continue
...
def main():
    ...
    if uploaded_file is not None:
        ...
        result = CLIENT.infer(img_bytes, model_id="medical_device_classification/2")
        device_type = result['predicted_classes'][0] if result['predicted_classes'] else None
        
        glucose_values, detected_device_type = preprocess_and_extract(temp_image_path)
        
        if glucose_values and detected_device_type:
            st.write("### Detected Values:")
            for value in glucose_values:
                st.write(f"Device: {device_type}, Value: {value}")
                new_row = {'Image': uploaded_file.name, detected_device_type: value}
                all_device_values.loc[len(all_device_values)] = new_row
